  ...
  module Opcode = struct
    ...
    let to_string = function
      ...
      | Pong ->
          "pong"
      | Ctrl i ->
          "ctrl " ^ string_of_int i
      | Nonctrl i ->
          "nonctrl " ^ string_of_int i

    let pp ppf t = Format.fprintf ppf "%s" (to_string t)

    let of_enum = function
      | i when i < 0 || i > 0xf ->
          invalid_arg "Frame.Opcode.of_enum"
      | 0 ->
          Continuation
      | 1 ->
          Text
  ...
  module Make (IO : Cohttp.S.IO) = struct
    ...
    let make_read_frame ?(buf = Buffer.create 128) ~mode ic oc () =
      ...
      | Some hdr -> (
          ...
          | 126 -> (
              read_uint16 ic buf
              >>= function Some i -> IO.return i | None -> IO.return @@ -1 )
          | 127 -> (
              read_int64 ic buf
              >>= function Some i -> IO.return i | None -> IO.return @@ -1 )
          | _ ->
          ...
          if payload_len = -1 then
            ...
            ( if frame_masked then (
              ...
              >>= function
              | None ->
                  raise (Protocol_error "could not read mask")
              | Some mask ->
                  IO.return mask )
