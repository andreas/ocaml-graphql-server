...
type variable_definition =
  {name: string; typ: typ; default_value: const_value option}
...
module Pp = struct
  ...
  let rec pp_value fmt = function
    ...
    | `Float f ->
        Fmt.float fmt f
    | `String s ->
        Fmt.(quote string) fmt (quote_string s)
    | `Bool b ->
        Fmt.bool fmt b
    | `Enum e ->
        Fmt.string fmt e
    | `Variable s ->
        Fmt.fmt "$%s" fmt s
    | `List l ->
        Fmt.(brackets (list ~sep:comma pp_value)) fmt l
    | `Assoc props ->
  ...
  let rec pp_typ fmt = function
    | NamedType t ->
        Fmt.string fmt t
    | ListType t ->
        Fmt.brackets pp_typ fmt t
    | NonNullType t ->
        Fmt.fmt "%a!" fmt pp_typ t
  ...
  let pp_optype fmt = function
    | Query ->
        Fmt.string fmt "query"
    | Mutation ->
        Fmt.string fmt "mutation"
    | Subscription ->
        Fmt.string fmt "subscription"
