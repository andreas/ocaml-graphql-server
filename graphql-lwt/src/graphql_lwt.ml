...
module Server = struct
  ...
  let execute_request ctx schema body =
    ...
    | Ok (`Stream _) ->
        C.Server.respond_string
          ~status:`Not_implemented
          ~body:"Subscription requests can't be fulfilled by this server."
          ()
    | Error err ->
        let body = Yojson.Basic.to_string err in
        C.Server.respond_string ~status:`Internal_server_error ~body ()

  let mk_callback mk_context schema _ (req : Cohttp.Request.t) body =
    Lwt_io.printf "Req: %s\n" req.resource |> Lwt.ignore_result;
    ...
    let path_parts = Str.(split (regexp "/") req_path) in
      ...
      | `POST, ["graphql"]       -> execute_request (mk_context req) schema body
