...
module Make (Io : IO) (Field_error : Field_error) = struct
  module Io = struct
    ...
    let rec all = function
      | [] ->
          Io.return []
      | x :: xs ->
          bind (all xs) (fun xs' -> map x ~f:(fun x' -> x' :: xs'))
  ...
  module Arg = struct
    ...
    type _ arg_typ =
      ...
      | Object :
          ...
          -> 'a option arg_typ
      | Enum :
          {name: string; doc: string option; values: 'a enum_value list}
          -> 'a option arg_typ
    ...
    let rec string_of_arg_typ : type a. a arg_typ -> string = function
      ...
      | Enum a ->
          a.name
      | List a ->
          Printf.sprintf "[%s]" (string_of_arg_typ a)
      | NonNullable a ->
          Printf.sprintf "%s!" (string_of_arg_typ a)
    ...
    let eval_arg_error ?(field_type = "field") ~field_name ~arg_name arg_typ
      ...
      let found_str =
        match value with
        | Some v ->
            Printf.sprintf "found %s" (string_of_const_value v)
        | None ->
            "but not provided"
      in
    ...
    let rec value_to_const_value variable_map = function
      ...
      | `Enum _ as e ->
          e
      | `Variable v ->
          StringMap.find_exn v variable_map
      | `List xs ->
          `List (List.map (value_to_const_value variable_map) xs)
      | `Assoc props ->
          let props' =
            List.map
              (fun (name, value) ->
            ...
            lazy
              [ Field
                  ...
                  ; resolve=
                      (fun _ t ->
                        ...
                        | AnyTyp (Enum _) ->
                            `Enum
                        | AnyTyp (NonNullable _) ->
                            `NonNull
                        | AnyArgTyp (Arg.Object _) ->
                            `InputObject
                        | AnyArgTyp (Arg.List _) ->
                            `List
                        | AnyArgTyp (Arg.Scalar _) ->
                            `Scalar
                        | AnyArgTyp (Arg.Enum _) ->
                            `Enum
                        | AnyArgTyp (Arg.NonNullable _) ->
                            `NonNull) }
              ; Field
                  { name= "name"
                  ; doc= None
              ...
              ; Field
                  ...
                  ; resolve=
                      (fun _ t ->
                        match t with
                        | AnyTyp (NonNullable typ) ->
                            Some (AnyTyp typ)
                        | AnyTyp (List typ) ->
                            Some (AnyTyp typ)
                        | AnyArgTyp (Arg.NonNullable typ) ->
                            Some (AnyArgTyp typ)
                        | AnyArgTyp (Arg.List typ) ->
                            Some (AnyArgTyp typ)
                        | _ ->
                            None) }
              ; Field
              ...
              [ Field
                  ...
                  ; resolve=
                      (fun _ f ->
                        ...
                        | AnyField (Field f) ->
                            f.name
                        | AnyArgField (Arg.Arg a) ->
                            a.name
                        | AnyArgField (Arg.DefaultArg a) ->
                            a.name) }
              ; Field
                  { name= "description"
                  ; doc= None
  ...
  let error_to_json ?path ?extensions msg =
    let props =
      match path with
      | Some path ->
          [("path", `List (List.rev path :> json list))]
      | None ->
          []
    in
    let extension_props =
  ...
  let rec present :
   ...
   fun ctx src query_field typ path ->
    match typ with
    | Scalar s ->
        coerce_or_null src (fun x -> Io.ok (s.coerce x, []))
    | List t ->
    ...
    | NonNullable t ->
        present ctx (Some src) query_field t path
    | Object o ->
        coerce_or_null src (fun src' ->
  ...
  let execute_operation :
   ...
   fun schema ctx operation ->
    ...
    | Graphql_parser.Mutation -> (
      ...
      | None ->
          Io.error `Mutations_not_configured
      | Some mut ->
    ...
    | Graphql_parser.Subscription -> (
      ...
      | None ->
          Io.error `Subscriptions_not_configured
      | Some subs -> (
          Io.return
            (collect_fields ctx
